...
//Header
  procedure glaciationAdv;
  procedure glaciationPers;
...
procedure glaciationAdv;
  begin
    ...
    text4 := '';
      if (precision = 0) then
        begin
          text3 := 'Votre adversaire effectu mal son sort "Glaciation" et meurt!';
          degatPers := 0;
    ...
    aleatoire(0, (nivAdv-nivCombat+30));
      if (precision <= lireIntegerRam(0)) and (nivAdv >= nivCombat) then
        begin
          text3 := 'Votre adversaire vous refroidis à -273°c et vous meurez.';
  ...
  begin
    ...
    aleatoire(0, (nivCombat-nivAdv+30));
      if (precision <= lireIntegerRam(0)) and (nivCombat >= nivAdv) then
        begin
          text1 := 'Vous refroidissez votre adversaire à -273°c et il meurt.';
          degatPers := 0;
          degatAdv := pvAdvActuel;
