...
//Header
  procedure ondeDeChocAdv;
  procedure ondeDeChocPers;
  ...
  begin
    ...
    chanceStatut := lireIntegerRam(0);
      if (chanceStatut = 1) and (statutPers = '') then
        begin
          text4 := 'L´onde de choc vous paralyse.';
          statutPers := 'Paralysé';
          ecrireStringRam(15, statutPers);
        end;
      if (precision = 0) then
        begin
          text3 := 'Pendant que votre adversaire crée un onde de choc, Il se loupe.';
          degatPers2 := 0;
      ...
      if (precision > 1) and (precision <= 90) then
        begin
          if efficaciteAdv = 1 then text3 := 'Votre adversaire lance un onde de choc. Pas très effiace.';
          if efficaciteAdv = 2 then text3 := 'Votre adversaire lance un onde de choc.';
          if efficaciteAdv = 4 then text3 := 'Votre adversaire lance un onde de choc. Très efficace.';
  ...
  begin
    ...
    chanceStatut := lireIntegerRam(0);
      if (chanceStatut = 1) and (statutAdv = '') then
        begin
          text2 := 'L´onde de choc paralyse votre adversaire.';
          statutAdv := 'Paralysé';
          ecrireStringRam(14, statutAdv);
        end;
      if (precision = 0) then
        begin
          text1 := 'Pendant que vous essayez de créer un onde de choc, vous vous loupez.';
      ...
      if (precision > 1) and (precision <= 90) then
        begin
          if efficacitePers = 1 then text1 := 'Vous lancez un onde de choc. Pas très efficace.';
          if efficacitePers = 2 then text1 := 'Vous lancez un onde de choc.';
          if efficacitePers = 4 then text1 := 'Vous lancez un onde de choc. Très efficace.';
          degatPers := 0;
