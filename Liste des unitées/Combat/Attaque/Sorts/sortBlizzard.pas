...
//Header
  procedure blizzardAdv;
  procedure blizzardPers;
  ...
  begin
    ...
    chanceStatut := lireIntegerRam(0);
      if (chanceStatut = 1) and (statutPers = '') then
        begin
          text4 := 'Le blizzard vous gêle.';
          statutPers := 'Gêlé';
          ecrireStringRam(15, statutPers);
        end;
      if (precision = 0) then
        begin
          text3 := 'Pendant que votre adversaire crée un blizzard, Il se loupe.';
          degatPers2 := 0;
      ...
      if (precision > 1) and (precision <= 90) then
        begin
          if efficaciteAdv = 1 then text3 := 'Votre adversaire lance un blizzard. Ce n´est pas très effiace.';
          if efficaciteAdv = 2 then text3 := 'Votre adversaire lance un blizzard.';
          if efficaciteAdv = 4 then text3 := 'Votre adversaire lance un blizzard. C´est très efficace.';
  ...
  begin
    ...
    chanceStatut := lireIntegerRam(0);
      if (chanceStatut = 1) and (statutAdv = '') then
        begin
          text2 := 'Le blizzard gêle votre adversaire.';
          statutAdv := 'Gêlé';
          ecrireStringRam(14, statutAdv);
        end;
      if (precision = 0) then
        begin
          text1 := 'Pendant que vous essayez de créer un blizzard, vous vous loupez.';
      ...
      if (precision > 1) and (precision <= 90) then
        begin
          if efficacitePers = 1 then text1 := 'Vous lancez un blizzard. Ce n´est pas très efficace.';
          if efficacitePers = 2 then text1 := 'Vous lancez un blizzard.';
          if efficacitePers = 4 then text1 := 'Vous lancez un blizzard. C´est très efficace.';
          degatPers := 0;
