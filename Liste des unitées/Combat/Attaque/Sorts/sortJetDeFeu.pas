...
//Header
  procedure jetDeFeuAdv;
  procedure jetDeFeuPers;
  ...
  begin
    ...
    chanceStatut := lireIntegerRam(0);
      if (chanceStatut = 1) and (statutPers = '') then
        begin
          text4 := 'Le jet de feu vous brule.';
          statutPers := 'Brulé';
          ecrireStringRam(15, statutPers);
        end;
      if (precision = 0) then
        begin
          text3 := 'Pendant que votre adversaire crée un jet de feu, Il se loupe.';
          degatPers2 := 0;
      ...
      if (precision > 1) and (precision <= 90) then
        begin
          if efficaciteAdv = 1 then text3 := 'Votre adversaire lance un jet de feu. Ce n´est pas très effiace.';
          if efficaciteAdv = 2 then text3 := 'Votre adversaire lance un jet de feu.';
          if efficaciteAdv = 4 then text3 := 'Votre adversaire lance un jet de feu. C´est très efficace.';
  ...
  begin
    ...
    chanceStatut := lireIntegerRam(0);
      if (chanceStatut = 1) and (statutAdv = '') then
        begin
          text2 := 'Le jet de feu brule votre adversaire.';
          statutAdv := 'Brulé';
          ecrireStringRam(14, statutAdv);
        end;
      if (precision = 0) then
        begin
          text1 := 'Pendant que vous essayez de créer un jet de feu, vous vous loupez.';
      ...
      if (precision > 1) and (precision <= 90) then
        begin
          if efficacitePers = 1 then text1 := 'Vous lancez un jet de feu. Ce n´est pas très efficace.';
          if efficacitePers = 2 then text1 := 'Vous lancez un jet de feu.';
          if efficacitePers = 4 then text1 := 'Vous lancez un jet de feu. C´est très efficace.';
          degatPers := 0;
