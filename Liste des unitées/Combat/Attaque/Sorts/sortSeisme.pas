...
procedure seismeAdv;
  begin
    ...
    efficaciteAdv := lireIntegerRam(53);
      if (precision = 0) then
        begin
          text3 := 'Pendant que votre adversaire crée un séisme, Il se loupe.';
          degatPers2 := 0;
      ...
      if (precision > 1) and (precision <= 90) then
        begin
          if efficaciteAdv = 1 then text3 := 'Votre adversaire lance un séisme. Ce n´est pas très effiace.';
          if efficaciteAdv = 2 then text3 := 'Votre adversaire lance un séisme.';
          if efficaciteAdv = 4 then text3 := 'Votre adversaire lance un séisme. C´est très efficace.';
...
procedure seismePers;
  begin
    ...
    aleatoire(1,20);
      if (precision = 0) then
        begin
          text1 := 'Pendant que vous essayez de créer un séisme, vous vous loupez.';
      ...
      if (precision > 1) and (precision <= 90) then
        begin
          if efficacitePers = 1 then text1 := 'Vous lancez un séisme. Ce n´est pas très efficace.';
          if efficacitePers = 2 then text1 := 'Vous lancez un séisme.';
          if efficacitePers = 4 then text1 := 'Vous lancez un séisme. C´est très efficace.';
          degatPers := 0;
